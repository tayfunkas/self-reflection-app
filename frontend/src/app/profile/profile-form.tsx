...
export default function ProfileForm({
  ...
  const [currentTimezone, setCurrentTimezone] = useState(timezone || "UTC");
  const [dailyEmailsEnabled, setDailyEmailsEnabled] =
    useState(dailyQuestionEmails);
    ...
    <div className="space-y-6">
      ...
      <form action={updatePreferences}>
        ...
        <section className="rounded-[28px] bg-[#FBF6F0] px-7 py-8 md:px-8 md:py-9">
          ...
          <div className="mt-6 flex flex-col gap-4 rounded-2xl border border-[#E8D9CC] bg-[#FFFDFC] px-5 py-4 md:flex-row md:items-center md:justify-between">
            <div>
              <p className="text-sm text-[#9A7D68]">
                Timezone used for daily reflections
              </p>
              <p className="mt-2 text-[16px] leading-7 text-[#705847]">
                {currentTimezone}
              </p>
              <p className="mt-1 text-xs leading-5 text-[#A08A79]">
                This is detected from your browser and saved when you update
                your preferences.
              </p>
            </div>
            ...
            <button
              type="submit"
              className="self-start rounded-full bg-[#6A4F3D] px-5 py-3 text-sm font-medium text-white transition hover:opacity-90 md:self-auto"
            >
              Save preferences
            </button>
          </div>
